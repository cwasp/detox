...
class DetoxServer {
        ...
        try {
          ...
          if (action.type === 'login') {
            if (action.params && action.params.sessionId && action.params.role) {
              ...
              log.log('debug', `${now()}:`, `role=${role} login (sessionId=${sessionId})`);
              _.set(this.sessions, [sessionId, role], ws);
              action.type = 'loginSuccess';
              this.sendAction(ws, action);
              log.log('debug', `${now()}:`, `role=${role} action=${action.type} (sessionId=${sessionId})`);
            }
          ...
          log.log('debug', `Invalid JSON received, cannot parse`, error);
        }
      });

      ws.on('close', () => {
        if (sessionId && role) {
          log.log('debug', `${now()}:`, `role=${role} disconnect (sessionId=${sessionId})`);
  ...
  sendToOtherRole(sessionId, role, action) {
    ...
    if (ws) {
      ...
      log.log('debug', `${now()}:`, `role=${otherRole} not connected, cannot fw action (sessionId=${sessionId})`);
    }
  }
